...
const CARD_BG = "rgba(14, 16, 22, 0.90)";
...
function TopBar({
  ...
  return (
    <div className="flex items-center justify-between px-5 pt-4 pb-2">
      <button
        onClick={handleBack}
        className="w-9 h-9 rounded-xl bg-zinc-900/90 border border-zinc-700/60 flex items-center justify-center text-zinc-200 hover:text-amber-400 hover:border-amber-500/50 transition-all active:scale-95 shadow-lg cursor-pointer"
      >
        <ChevronLeft size={18} />
      </button>
      <div className="text-center">
        <div className="text-sm sm:text-base font-black text-zinc-100 tracking-tight">{title}</div>
        {stepText && <div className="text-[10px] font-bold text-amber-400/90 uppercase tracking-widest">{stepText}</div>}
      </div>
...
function Card({
  ...
  return (
    ...
    >
      {selected && (
        <div className="absolute top-2.5 right-2.5 w-4 h-4 rounded-full bg-amber-400 flex items-center justify-center text-black shadow-md">
...
function BottomBar({ children }: { children: React.ReactNode }) {
  return (
    <div
      className="sticky bottom-0 left-0 right-0 w-full px-5 pt-2.5 pb-[calc(14px+env(safe-area-inset-bottom))] z-40"
      style={{
  ...
  return (
    <div className="min-h-full flex flex-col justify-between">
      ...
      <div className="px-5 pt-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-900/90 border border-amber-500/30 backdrop-blur-md shadow-lg">
      ...
      <div className="px-5 pt-4 text-left">
        <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-300 text-[11px] font-black uppercase tracking-wider mb-2">
      ...
      <div className="px-5 flex flex-col gap-2.5">
        ...
        <button
          ...
          className="w-full py-3 rounded-2xl bg-zinc-900/80 border border-zinc-800 text-zinc-300 font-bold text-xs hover:bg-zinc-800 transition-all cursor-pointer backdrop-blur-md"
        >
      ...
      <div className="px-5 mt-5">
        <div className="grid grid-cols-3 gap-2 p-2.5 rounded-2xl bg-zinc-950/70 border border-zinc-800/80 text-center">
          <div>
            <div className="text-sm font-black text-amber-400">4.9★</div>
            <div className="text-[9.5px] font-bold text-zinc-400 uppercase tracking-tight">4,800+ Bookings</div>
          </div>
          <div className="border-x border-zinc-800">
            <div className="text-sm font-black text-amber-400">&lt; 3m</div>
            <div className="text-[9.5px] font-bold text-zinc-400 uppercase tracking-tight">Fastest Match</div>
          </div>
          <div>
            <div className="text-sm font-black text-emerald-400">100%</div>
            <div className="text-[9.5px] font-bold text-zinc-400 uppercase tracking-tight">Police Verified</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 pt-6 pb-6">
        <div className="text-[10.5px] font-black text-zinc-400 tracking-wider uppercase mb-2.5">
          HOW IT WORKS
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {[
            ...
            <div
              key={item.step}
              className="flex items-center gap-2.5 p-2.5 rounded-xl bg-zinc-900/50 border border-zinc-800/50"
            >
              <div className="w-6 h-6 rounded-lg bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-[11px] font-black text-amber-400">
                {item.step}
              </div>
              <div>
...
function HeroRadarVisualizer() {
  const nearbyBouncers = [
    { name: "Priya S.", eta: "2m", isFemale: true, angle: 35, dist: 74 },
    { name: "Vikram R.", eta: "3m", isFemale: false, angle: 140, dist: 80 },
    { name: "Sanya V.", eta: "4m", isFemale: true, angle: 220, dist: 72 },
    { name: "Arjun K.", eta: "4m", isFemale: false, angle: 310, dist: 84 },
  ];

  return (
    <div className="relative w-full h-full flex items-center justify-center">
      {[60, 110, 160, 220].map((size, idx) => (
        <div
          key={idx}
          ...
          style={{
            width: size,
            height: size,
            boxShadow: idx === 1 ? "0 0 25px rgba(245, 158, 11, 0.08)" : "none",
          }}
        />
      ))}

      <div
        className="absolute w-56 h-56 rounded-full pointer-events-none animate-[spin_5s_linear_infinite]"
        style={{
          background:
      ...
      <div className="relative z-10 w-14 h-14 rounded-full bg-gradient-to-tr from-amber-600 to-yellow-400 flex items-center justify-center shadow-[0_0_35px_rgba(245,158,11,0.8)] border-2 border-white/80">
      ...
      {nearbyBouncers.map((b, i) => {
        ...
        return (
          <div
            key={i}
            className="absolute z-20 flex items-center gap-1.5 px-2 py-1 rounded-xl bg-black/90 border border-amber-500/40 shadow-xl backdrop-blur-md"
            style={{
              left: `calc(50% + ${x}px)`,
              top: `calc(50% + ${y}px)`,
        ...
        {PURPOSES.map((p) => {
          ...
          return (
            <Card
              ...
              className="flex flex-col justify-between min-h-[105px]"
            >
              <div>
                <div
              ...
              <div className="text-[10px] text-zinc-400 mt-1 leading-tight">{p.desc}</div>
            </Card>
          );
        })}
      </div>
  ...
  return (
    <div>
      ...
      <div className="px-5 pb-6">
        ...
        {/* Live Visual Squad Lineup with Tactical Arm Protection & Avatars */}
        <div className="flex items-center justify-center gap-2 py-3 mb-4 rounded-2xl bg-zinc-950/60 border border-zinc-800">
          ...
          {count > 5 && (
            <div className="w-10 h-10 rounded-full bg-amber-500/20 border border-amber-400 flex items-center justify-center text-xs font-black text-amber-300">
              +{count - 5}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between p-3 rounded-2xl bg-zinc-900 border border-zinc-800 mb-4">
          <button
            ...
            className="w-10 h-10 rounded-xl bg-zinc-800 border border-zinc-700 flex items-center justify-center text-white active:scale-90 transition-all font-black cursor-pointer"
          >
            <Minus size={16} />
          </button>
          <div className="text-center">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-black text-white">{lvl.label}</span>
                    <span className="px-2 py-0.5 rounded-full bg-amber-400/20 text-amber-300 text-[9.5px] font-black uppercase tracking-wider">
                      {lvl.tag}
                    </span>
                  </div>
                  <div className="text-[11px] text-zinc-400 mt-0.5">{lvl.desc}</div>
                </div>
                <div className="text-right flex-shrink-0">
                  <div className="text-base font-black text-amber-400">₹{lvl.rate}</div>
                  <div className="text-[9.5px] text-zinc-500 font-bold">/ hour</div>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-1 pt-1.5 border-t border-zinc-800/80">
                {lvl.features.map((f, i) => (
                  <div key={i} className="flex items-center gap-1 text-[10.5px] text-zinc-300">
            ...
            {PRESENCE.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`text-[8.5px] mt-0.5 ${sel ? "text-black/70 font-semibold" : "text-zinc-500"}`}>
                    {p.sub}
                  </div>
                </button>
              );
          ...
          <div className="grid grid-cols-2 gap-2">
            {GENDERS.map((g) => {
              ...
              return (
                ...
                >
                  {g.id === "female" && <FemaleBouncerAvatar className="w-12 h-12 mb-2" />}
                  {g.id === "male" && <MaleBouncerAvatar className="w-12 h-12 mb-2" />}
                  {g.id === "mixed" && <MixedTeamAvatar className="w-12 h-12 mb-2" />}
                  {g.id === "any" && (
                  ...
                  <div className="text-[9.5px] text-zinc-400 mt-0.5">{g.desc}</div>
                </Card>
              );
            })}
          </div>
  ...
  return (
    <div>
      ...
      <div className="px-5 pb-6 space-y-3">
        ...
        {/* Hotspots */}
        <div>
          <div className="text-[10.5px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none">
            {HOTSPOT_LOCATIONS.map((h) => (
              <button
                ...
                className="px-2.5 py-1 rounded-full bg-zinc-900 border border-zinc-800 text-[10.5px] font-bold text-zinc-300 whitespace-nowrap hover:border-amber-400 hover:text-amber-300 transition-all active:scale-95 cursor-pointer"
              >
                📍 {h.name}
              </button>
            ))}
        ...
        <Card className="flex items-center gap-2.5 bg-zinc-900/90 py-2.5">
          <div className="w-8 h-8 rounded-lg bg-zinc-800 flex items-center justify-center text-amber-400 flex-shrink-0">
            <MapPin size={18} />
          </div>
  ...
  return (
    <div>
      ...
      <div className="px-5 pb-6 space-y-3.5">
        <div>
          <div className="text-[11px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">DATE</div>
          <input
            type="date"
            value={date}
            onChange={(e) => {
            ...
            className="w-full bg-zinc-900 border border-zinc-700/80 rounded-xl px-3.5 py-2.5 text-xs text-zinc-100 font-bold focus:outline-none focus:border-amber-400"
          />
        </div>

        <div>
          <div className="text-[11px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">START TIME</div>
        ...
        <div>
          <div className="text-[11px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">
        ...
        {date && (
          <Card className="flex items-center justify-center gap-2 py-2.5 bg-amber-500/10 border-amber-500/30">
    ...
    <div>
      ...
      <div className="px-5 pb-6 space-y-3">
        <Card className="space-y-2">
          <div className="text-[10px] font-black text-amber-400 tracking-wider uppercase">
            BOOKING DETAILS
          </div>
          {[
            ...
            <div key={k} className="flex justify-between items-center py-0.5 border-b border-zinc-800/60 text-xs">
              <span className="text-zinc-400">{k}</span>
              <span className="text-white font-bold text-right truncate max-w-[180px]">{v}</span>
            </div>
          ))}
        </Card>
        ...
        <Card className="space-y-1.5">
          ...
          <div className="flex justify-between items-center pt-2 border-t border-zinc-800">
            <span className="text-xs font-black text-white">Total Escrow Amount</span>
            <span
        ...
        <div className="flex items-center gap-2 p-2.5 rounded-xl bg-zinc-900 border border-zinc-800 text-[10.5px] text-zinc-300">
    ...
    <div className="px-5 pt-6 pb-6">
      <div className="text-center mb-4">
        ...
        <div className="text-2xl font-black text-white mt-0.5">
          {accepted.length} / {needed} <span className="text-zinc-400 text-sm font-bold">Confirmed</span>
        </div>
      </div>

      <div className="relative w-44 h-44 mx-auto mb-4 flex items-center justify-center">
        <svg className="w-44 h-44 absolute inset-0 -rotate-90">
          <circle cx="88" cy="88" r="76" fill="none" stroke="#1E222D" strokeWidth="8" />
          <circle
            cx="88"
            cy="88"
            r="76"
        ...
        <div className="w-32 h-32 rounded-full bg-zinc-950 border border-zinc-800 flex flex-col items-center justify-center shadow-inner">
          <RadarIcon size={26} className="text-amber-400 animate-spin" />
          <div className="text-[10px] font-black text-zinc-300 mt-1.5">{totalNotified} Notified</div>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="p-2.5 rounded-xl bg-zinc-900/80 border border-zinc-800 text-center">
          <div className="text-lg font-black text-emerald-400">{accepted.length}</div>
          <div className="text-[9px] font-bold text-zinc-400 uppercase">Accepted</div>
        </div>
        <div className="p-2.5 rounded-xl bg-zinc-900/80 border border-zinc-800 text-center">
          <div className="text-lg font-black text-rose-400">{rejected}</div>
          <div className="text-[9px] font-bold text-zinc-400 uppercase">Declined</div>
        </div>
        <div className="p-2.5 rounded-xl bg-zinc-900/80 border border-zinc-800 text-center">
          <div className="text-lg font-black text-zinc-300">{waiting}</div>
          <div className="text-[9px] font-bold text-zinc-400 uppercase">Waiting</div>
        </div>
      </div>
...
function TeamConfirmedScreen({
  ...
  return (
    <div className="px-5 pt-6 pb-6 text-center">
      <div className="w-14 h-14 rounded-full bg-gradient-to-tr from-amber-600 to-yellow-400 flex items-center justify-center mx-auto mb-3 shadow-[0_10px_35px_rgba(245,158,11,0.6)]">
        <CheckCircle2 size={32} className="text-black" />
      </div>
      <h2 className="text-xl font-black text-white">Your Team is Confirmed!</h2>
      <p className="text-xs text-zinc-400 mt-0.5 max-w-[300px] mx-auto">
        {team.length} verified security captains assigned & dispatching.
      </p>

      {/* Security OTP Start PIN */}
      <div className="mt-4 p-3 rounded-xl bg-amber-500/10 border border-amber-500/40 text-center">
        <div className="text-[10px] font-black text-amber-400 uppercase tracking-widest">
          SECURITY START PIN
        </div>
        <div className="text-2xl font-black text-white tracking-widest mt-0.5">4 8 9 2</div>
        <div className="text-[9.5px] text-zinc-400 mt-0.5">Share this OTP with captains upon arrival</div>
      </div>
      ...
      <div className="mt-4 space-y-2 text-left">
        {team.map((m) => (
          <Card key={m.id} className="flex items-center gap-3 py-2.5">
            ...
            <div className="px-2 py-0.5 rounded-full bg-amber-400/20 text-amber-300 text-[10px] font-black">
              ETA {rand(4, 7)}m
            </div>
          </Card>
        ))}
  ...
  return (
    <div className="pb-6">
      ...
      <div className="px-5 mt-3 space-y-2">
        {team.map((m) => (
          <Card key={m.id} className="flex items-center gap-3 py-2">
            {m.gender === "female" ? (
              <FemaleBouncerAvatar className="w-10 h-10" />
            ) : (
              <MaleBouncerAvatar className="w-10 h-10" />
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs font-black text-white truncate">{m.name}</div>
              <div className="text-[10px] text-zinc-400">★ {m.rating} • En Route (Arm Guards)</div>
            </div>
            <div className="flex gap-1.5">
              <button
                onClick={() => toast(`Calling ${m.name} (+91 98xxx)...`)}
                className="w-8 h-8 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center text-amber-400 hover:bg-amber-400 hover:text-black transition-all cursor-pointer"
              >
                <Phone size={14} />
              </button>
              <button
                onClick={() => toast(`Opening live chat with ${m.name}`)}
                className="w-8 h-8 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center text-zinc-200 hover:bg-zinc-700 transition-all cursor-pointer"
              >
      ...
      <div className="px-5 mt-3">
        <Card className="space-y-1 text-xs py-2">
          <div className="text-[10px] font-black text-amber-400 uppercase tracking-wider">
            DETAILS
          </div>
          <div className="flex justify-between text-zinc-400">
            <span>Location:</span>
            <span className="text-white font-bold truncate max-w-[180px]">{booking.address}</span>
          </div>
          <div className="flex justify-between text-zinc-400">
      ...
      <div className="px-5 mt-3">
        <button
          ...
          className="w-full py-3 rounded-xl bg-rose-950/80 border border-rose-600/60 text-rose-300 font-black text-xs flex items-center justify-center gap-2 hover:bg-rose-900 transition-all shadow-lg cursor-pointer"
        >
...
function RatingScreen({ team, onDone }: { team: any[]; onDone: () => void }) {
  ...
  return (
    <div className="px-5 pt-6 pb-6 text-center">
      <div className="w-12 h-12 rounded-full bg-amber-500/10 border border-amber-500/30 flex items-center justify-center mx-auto mb-2.5 text-amber-400">
        <Award size={26} />
      </div>
      ...
      <div className="space-y-2 text-left">
        {cats.map((c) => (
          <div key={c} className="flex justify-between items-center p-2.5 rounded-xl bg-zinc-900 border border-zinc-800">
...
/* -------------------------------------------------------------------------
   APP ROOT (Optimized Frame on Laptop + Full Screen on Mobile)
------------------------------------------------------------------------- */
...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen bg-[#050608] text-zinc-100 flex flex-col items-center justify-center selection:bg-amber-400 selection:text-black font-sans overflow-x-hidden p-0 sm:p-4 md:p-6">
      {/* 1. Interactive WebGL Shader Animated Background (Obsidian & Gold Cyber Clouds) */}
      <ShaderBackgroundCanvas />

      {/* 2. Perfectly Proportioned Mobile Showcase Container on Laptop / Desktop & Full Screen on Phone */}
      <div className="relative z-10 w-full flex items-center justify-center">
        <main className="w-full sm:max-w-[430px] min-h-screen sm:min-h-[860px] sm:max-h-[920px] bg-[#090b10]/95 border-0 sm:border sm:border-zinc-800/80 rounded-none sm:rounded-[36px] shadow-[0_0_80px_rgba(0,0,0,0.95),0_0_30px_rgba(245,158,11,0.15)] backdrop-blur-2xl overflow-y-auto overflow-x-hidden flex flex-col transition-all duration-300 scrollbar-none">
          {content}

          {/* Floating Toast Notification */}
          {toastMsg && (
            <div className="fixed bottom-20 max-w-xs mx-auto left-4 right-4 bg-zinc-900 border border-amber-500/40 text-white text-xs font-bold px-3.5 py-2.5 rounded-xl shadow-2xl z-50 animate-fade-in flex items-center gap-2">
